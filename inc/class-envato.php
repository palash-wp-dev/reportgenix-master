...
if (!class_exists('Xgenious_Envato')){
	class Xgenious_Envato{
		//$instance variable
		...
		public static function get_product_rating_by_id($item_id,$post_id){
		    $query_args = http_build_query(['id' => $item_id]);
		    $retur_val = [ 'rating' => 5,'rating_count' => 1];
			$response = wp_remote_get("https://api.envato.com/v3/market/catalog/item?".$query_args,array(
				'headers' => [
					'User-Agent'=> $_SERVER['HTTP_USER_AGENT'],
					'Authorization'=> 'Bearer '. cs_get_option('envato_api_token')
			...
			if (is_array($response) && !is_wp_error($response)){
				...
				$retur_val['rating'] = property_exists($body,'rating') && $body->rating > 0 ? $body->rating : 5;
				$retur_val['rating_count'] = property_exists($body,'rating_count') && $body->rating_count > 0 ? $body->rating_count : 1;
				update_post_meta($post_id,'xgenious_rating',$retur_val['rating']);
				update_post_meta($post_id,'xgenious_rating_count',$retur_val['rating_count'] );
		...
		public function extract_product_info_from_envato(){
			...
			if (is_array($response) && !is_wp_error($response)){
				$body = json_decode($response['body']);
				if ($body->error == 404){
					Xgenious()->set_flash_message('purchase_status',404);
					Xgenious()->set_flash_message('purchase_message', $body->description);
                ...
                if (property_exists($body,'previews') && !empty($post_id)){
                    ...
                    if (property_exists($preview_images,'icon_preview')){
                        ...
                        if (is_array($thumbnail_image_url)){
                            ...
                            add_post_meta($post_id,'xgenious_thumbnail',
                                array(
                ...
                if ($_POST['item_type'] == 'laravel'){
                    ...
                    if(!empty($post_id)){
                        wp_set_object_terms($post_id,'laravel','download_category');
                    }

                }elseif($_POST['item_type'] == 'wordpress-plugins'){
                    if (property_exists($body,'attributes')) {
                        $item_attributes = $body->attributes;
                        foreach ($item_attributes as $att){
                            $attrb = (array) $att;
                    ...
                    if(!empty($post_id)){
                        wp_set_object_terms($post_id,'wordpress-plugins','download_category');
                    }

                }elseif ($_POST['item_type'] == 'css-elements'){
                    ...
                    if(!empty($post_id)){
                        wp_set_object_terms($post_id,'css-elements','download_category');
                    }
                ...
                if (!empty($post_id)){
                    foreach ($xgenious_product_info as $key => $value){
                        add_post_meta($post_id,'xgenious_'.$key,$value );
                    }
                }


				Xgenious()->set_flash_message('purchase_status',200);
				Xgenious()->set_flash_message('extract_message',esc_html__('Product Extract & Imported Success','xgenious-master'));
				wp_safe_redirect($redirect_path);
		...
		public function envato_purchase_verify(){
			...
			if (is_array($response) && !is_wp_error($response)){
				...
				$purchase_verify_info = [
					'purchase_message' => esc_html__('Purchase Verify Success, Purchase Details Below','xgenious'),
					'buyer' => $body->buyer,
				...
				foreach ($purchase_verify_info as $key => $message){
					Xgenious()->set_flash_message($key,$message);
				}
				wp_safe_redirect($redirect_path);
			}
