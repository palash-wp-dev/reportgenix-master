...
class Xgenious_Advance_Tab_One_Widget extends Widget_Base {
	...
	public function get_name() {
		return 'xgenious-advnace-tab-one-widget';
	}
	...
	public function get_title() {
		return esc_html__( 'Advance Tab: 01', 'xgenious-master' );
	}
	...
	protected function render() {
		...
		?>
	<section class="features_area padding-top-100 padding-bottom-100">
        <div class="container">
            ...
            <div class="features_tab_wrapper center-text mt-5">
                <ul class="nav nav-pills tabs tab_style_one" role="tablist">
                     <?php
                     $tab_ids = [];
                        $job_list = $settings['right_faq_lists'];
                        foreach ($job_list as $key => $item):
                        $active = $key === 0 ? 'active' : '';
                        $tab_id = 'tabs_'.str_replace([' '],[''],strtolower(str_replace(['/','&'],'_',$item['tab_title']))).random_int(123456789,999999999);
                        $tab_ids[] = $tab_id;
...
Plugin::instance()->widgets_manager->register( new Xgenious_Advance_Tab_One_Widget() );