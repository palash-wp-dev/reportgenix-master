...
if ( ! class_exists( 'Xgenious_Admin_Menu' ) ) {

class Xgenious_Admin_Menu {
    /*
        * $instance
        * @since 1.0.0
        * */
   ...
   * */
    ...
    public function theme_admin_menu_page() {
        ...
        //add menu page
        add_menu_page(
            esc_html__( 'Xgenious Page', 'xgenious-master' ),
            esc_html__( 'Xgenious', 'xgenious-master' ),
            'manage_options',
            'xgenious_theme_options',
            array( $this, 'admin_optoins_fallback_function' ),
            XGENIOUS_MASTER_ADMIN_ASSETS . '/img/icon.png',
            80
        );
        add_submenu_page(
            'xgenious_theme_options',
            esc_html__( 'Product Extract', 'xgenious' ),
            esc_html__( 'Products Extract', 'xgenious' ),
            'manage_options',
        ...
        add_submenu_page(
            'xgenious_theme_options',
            esc_html__( 'Purchase Verify', 'xgenious' ),
            esc_html__( 'Purchase Verify', 'xgenious' ),
            'manage_options',
            'envato-purchase-verify',
            array( $this, 'envato_purchase_verify_display' )
        );
	    add_submenu_page(
            'xgenious_theme_options',
            esc_html__( 'Duplicate Doc', 'xgenious' ),
            esc_html__( 'Duplicate Doc', 'xgenious' ),
            'manage_options',
            'duplicate-wedocs',
            array( $this, 'duplicate_wedocs_display' )
        );
    }

    public function duplicate_wedocs_display() {
        //admin menu page
	    require_once XGENIOUS_MASTER_ADMIN . '/partials/duplicate-wedocs.php';
    }
...
//						'name' => sprintf(esc_html__('%s','xgenious-master'),'Practice Area'),
//						'id' => 'edit-practice-area'
//					),
//					array(
//						'link' => 'edit-tags.php?taxonomy=practice-cat&post_type=practice-area',
//						'name' => sprintf(esc_html__('%s Categories','xgenious-master'),'Practice Area'),
...
//			}
        ...
        ?>
            <script type="text/javascript">
                (function ($) {
                    ...
                    mainwrap = $('#toplevel_page_xgenious_theme_options');
                    ...
                    if (check) {
                        $('.wp-submenu.wp-submenu-wrap')
                            .find('a[href*="admin.php?page=toplevel_page_xgenious_theme_options"]')
                            .removeClass('current')
                            .parent('li')
                            .removeClass('current');
                    }
    ...
    public function envato_product_extrarct_display() {
        require_once XGENIOUS_MASTER_ADMIN . '/partials/envato-product-extract.php';
    }
    ...
    public function envato_purchase_verify_display() {
        require_once XGENIOUS_MASTER_ADMIN . '/partials/envato-purchase-verify.php';
    }

}//end class
if ( class_exists( 'Xgenious_Admin_Menu' ) ) {
    Xgenious_Admin_Menu::getInstance();
}
