...
class Xgenious_About_Us_Widget extends WP_Widget{

	public function __construct() {
		parent::__construct(
			'xgenious_about_us',
			esc_html__('Xgenious: About Us','xgenious-master'),
	...
	public function form($instance){
		...
		?>
		<p class="xgenious_sub_title"><?php esc_html_e('About Us Logo','xgenious-master')?></p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('bf_logo')); ?>"></label>
			<input type="hidden" class="widefat xgenious_logo_id"
			       id="<?php echo esc_attr($this->get_field_id('bf_logo')); ?>"
			       name="<?php echo esc_attr($this->get_field_name('bf_logo'))?>"
			       value="<?php if ( ! empty( $instance['bf_logo'] ) ) echo esc_attr($instance['bf_logo']);?>"
			/>
		<div class="xgenious-logo-preview"></div>
		<input type="file" class="xgenious_flogo_uploader" value="<?php esc_html_e('Upload Logo','xgenious-master');?>">
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_name('bf_description'))?>"><?php esc_html_e('About Widget Description','xgenious-master')?></label>
			<textarea name="<?php echo esc_attr($this->get_field_name('bf_description'))?>" id="<?php echo esc_attr($this->get_field_id('bf_description'))?>" cols="30" class="xgenious-form-control" rows="5"><?php echo esc_html($instance['bf_description'])?></textarea>
		</p>
...
if (!function_exists('Xgenious_About_Us_Widget')){
	function Xgenious_About_Us_Widget(){
		register_widget('Xgenious_About_Us_Widget');
	}
	add_action('widgets_init','Xgenious_About_Us_Widget');
}
